...
const login = async () => {
  ...
  try {
    ...
    console.error("Web3Auth FAILED → using fallback login mode:", error);

    // fallback LOGIN palsu
    const dummyUser = {
      email: "guest@example.com",
      name: "Guest User",
      address: "0xFAKE1234567890abcdefABCDE1234567890abcd",
      loginType: "fallback",
    };

    setProvider(null); // tidak pakai provider Web3Auth
    setUserInfo(dummyUser);
    setLoggedIn(true);

    localStorage.setItem("userEmail", dummyUser.email);

    // optional: buat user di database secara dummy
    try {
      await createUser(dummyUser.email, dummyUser.name);
    } catch (err) {
          ...
          <Button variant="ghost" size="icon" className="mr-2 md:mr-4" onClick={onMenuClick}>
            ...
            <Link href="/" className="flex items-center">
              ...
              <div className="flex flex-col">
                <span className="font-bold text-base md:text-lg text-gray-800">Jaga Biru</span>
                <span className="text-[8px] md:text-[10px] text-gray-500 -mt-1">Kelompok 13</span>
              </div>
            </Link>
        </div>
        {!isMobile && (
